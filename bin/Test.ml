...
let remove_comments l =
  ...
  let rec loop = function
    | true, Exclamations n :: tl when n > 0 ->
      loop (true,
            Utils.eat (function Newline|Newlines _ -> false|_-> true) tl)
...
let remove_endline_comments l =
  ...
  let rec loop = function
    ...
    | Exclamations n :: tl when n > 0 ->
      loop (Utils.eat (function Newline|Newlines _ -> false|_-> true) tl)
    | e::tl ->
      e::loop tl
    | [] -> []
  in loop l
...
let list_html_tags ~inline =
  let module Parser = Parser.Make(E)
  in
  if inline then
            ...
            | Verb x ->
              [Representation.Tag(
                  ...
                  object
                    method parser_extension r p l =
                      match p with
                      | [] | [Representation.Newlines _] ->
                        Some(Raw_block x :: r, [Representation.Space], l)
                      | _ ->
                        Some(Raw x :: r, [Representation.Space], l)
                    method to_string = x
                  end
                )]
          )
...
let main () =
  ...
  List.iter (fun ic ->
    ...
    with End_of_file ->
      ...
      let module E = Parser.Default_env(struct end) in
      let module Parser = Parser.Make(
        struct
          include E
      ...
      in
        ...
        if false && Utils.debug then
          print_endline
            (Backend.sexpr_of_md
               (Omd.Parser.default_parse
                  (preprocess(Lexer.lex (Buffer.contents b)))));
  )
    input_files
